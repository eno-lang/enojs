...
const unterminatedEscapedName = (context, instruction, unterminated) => {
  ...
  const message = context.messages.tokenization.unterminatedEscapedName(instruction.lineNumber);
  const snippet = report(context, instruction);
  const selection = [[instruction.lineNumber, selectionColumn], [instruction.lineNumber, instruction.line.length]]
