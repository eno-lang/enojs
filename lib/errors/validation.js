...
module.exports = {
  ...
  exactCountNotMet: (context, instruction, exactCount) => {
    ...
    if(instruction.subinstructions && instruction.subinstructions.length > 0) {
      ...
      selection = [
        [instruction.subinstructions[0].lineNumber, 0],
        [lastSubinstruction.lineNumber, lastSubinstruction.line.length]
      ];
  ...
  missingValue: (context, instruction) => {
    ...
    if(instruction.type === 'FIELD' || instruction.type === 'NAME' || instruction.type === 'BLOCK') {
      ...
      if(instruction.ranges.hasOwnProperty('template')) {
        selection = [[instruction.lineNumber, instruction.ranges.template[1]]];
      } else if(instruction.ranges.hasOwnProperty('nameOperator')) {
        selection = [[
          instruction.lineNumber,
          Math.min(instruction.ranges.nameOperator[1] + 1, instruction.line.length)
        ]];
      } else {
        selection = [[instruction.lineNumber, instruction.line.length]];
      }
      ...
      selection = [[
        instruction.lineNumber,
        Math.min(instruction.ranges.entryOperator[1] + 1, instruction.line.length)
      ]];
    } else if(instruction.type === 'LIST_ITEM') {
      message = context.messages.validation.missingListItemValue(instruction.name);
      selection = [[
        instruction.lineNumber,
        Math.min(instruction.ranges.itemOperator[1] + 1, instruction.line.length)
      ]];
    }
  ...
  valueError: (context, message, instruction) => {
    ...
    if(instruction.type === 'BLOCK') {
      ...
      if(instruction.ranges.hasOwnProperty('value')) {
        selection = [[instruction.lineNumber, instruction.ranges.value[0]]];
      } else if(instruction.ranges.hasOwnProperty('template')) {
        selection = [[instruction.lineNumber, instruction.ranges.templateOperator[0]]];
      } else if(instruction.ranges.hasOwnProperty('nameOperator')) {
        selection = [[
          instruction.lineNumber,
          Math.min(instruction.ranges.nameOperator[1] + 1, instruction.line.length)
      ...
      if(instruction.subinstructions && instruction.subinstructions.length > 0) {
        ...
        if(instruction.ranges.hasOwnProperty('value')) {
          selection.push([instruction.lineNumber, instruction.ranges.value[1]]);
        } else {
          selection.push([instruction.lineNumber, instruction.line.length]);
        }
      }
