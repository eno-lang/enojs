...
module.exports = {
  ...
  valueError: (context, message, valueInstruction, onlyGenerate = false) => {
    let messageProcessed;
    if(message) {
      if(typeof message === 'function') { // TODO: Clarify if this theoretical possibility has any actual usecase (also: what's "this" here?)
        messageProcessed = message(this);
      } else {
        messageProcessed = message;
      }
    } else {
      messageProcessed = context.messages.validation.genericError(valueInstruction.name);
    }
    ...
    const error = new EnoValidationError(messageProcessed, snippet, selection);
