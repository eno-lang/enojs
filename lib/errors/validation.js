...
module.exports = {
  ...
  excessName: (context, message, instruction) => {
    ...
    if(instruction.subinstructions && instruction.subinstructions.length > 0) {
      snippet = context.reporter.report(context, [instruction, ...instruction.subinstructions]);
  ...
  expectedFieldsetsGotField: (context, instruction) => {
    ...
    const snippet = context.reporter.report(context, [instruction, ...instruction.subinstructions]);
  ...
  expectedFieldsetGotFieldsets: (context, name, instructions) => {
    ...
    const snippet = context.reporter.report(context, instructions, expandedInstructions);
  ...
  missingFieldset: (context, name, sectionInstruction) => {
    ...
    const snippet = context.reporter.report(context, sectionInstruction, deepExpandInstruction(sectionInstruction));
  ...
  missingFieldsetEntry: (context, name, fieldsetInstruction) => {
    ...
    const snippet = context.reporter.report(context, fieldsetInstruction, deepExpandInstruction(fieldsetInstruction));
  ...
  missingValue: (context, instruction) => {
    ...
    const snippet = context.reporter.report(context, instruction, deepExpandInstruction(instruction));
  ...
  valueError: (context, message, instruction) => {
    ...
    if(instruction.type === 'BLOCK') {
      ...
      if(contentInstructions.length > 0) {
        ...
        snippet = context.reporter.report(context, contentInstructions);
        selection = [
          [firstInstruction.line, firstInstruction.ranges.content[0]],
          [lastInstruction.line, lastInstruction.ranges.content[1]]
        ];
      } else {
        snippet = context.reporter.report(context, [instruction, terminatorInstruction]);
        selection = [
          [instruction.line, instruction.length],
          [instruction.line, instruction.length]
        ];
