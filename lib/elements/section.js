...
const EnoFieldset = require('./fieldset.js');
...
class EnoSection {
  constructor(context, instruction, parent) {
    ...
    for(let subinstruction of instruction.subinstructions) {
      ...
      if(subinstruction.type === 'FIELDSET') {
        append(new EnoFieldset(context, subinstruction, this));
        continue;
      }
      ...
      for(let element of elements) {
        if(!element.touched) {
          if(typeof message === 'function') {
            const value = element instanceof EnoFieldset ||
        ...
        if(element instanceof EnoFieldset || element instanceof EnoSection) {
  ...
  fieldsets(name) {
    ...
    return elements.map(element => {
      ...
      if(element instanceof EnoFieldset) {
        return element;
      }

      if(element instanceof EnoEmpty) {
        return new EnoFieldset(this.context, element.instruction, this, true);
      }
...
EnoSection.prototype.string = EnoSection.prototype.field;
EnoSection.prototype.stringList = EnoSection.prototype.list;

for(let [name, func] of Object.entries(loaders)) {
  EnoSection.prototype[name] = function(name, ...optional) {
    return this.field(name, func, ...optional);
  }

  EnoSection.prototype[`${name}List`] = function(name, ...optional) {
