...
class EnoValue {
  constructor(context, instruction, parent, fromEmpty = false) {
    ...
    // A value might be lazily constructed if it appears in the document as an empty element,
    // which represents an ambiguous element that might resolve to either a dictionary, field or list.
    // When the application requests such an empty element as a field, it lazily constructs a value.
    if(!fromEmpty) {
      ...
      // TODO: Can't do this, trims off block content too
      if(this._value !== null) {
