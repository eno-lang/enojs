    ...
    if(instruction.type === 'BLOCK' && instruction.hasOwnProperty('contentRange')) {
      ...
      for(let subinstruction of instruction.subinstructions) {
        ...
        if(subinstruction.type === 'BLOCK') {
          if(this._value === null) {
            this._value = context.input.substring(subinstruction.contentRange[0], subinstruction.contentRange[1] + 1);
            unresolved_newlines = 0;
          } else {
            if(unresolved_newlines > 0) {
              this._value += '\n'.repeat(unresolved_newlines)
            ...
            this._value += '\n' + context.input.substring(subinstruction.contentRange[0], subinstruction.contentRange[1] + 1);
          }
          continue;
        }
      }
