...
class Field {
  ...
  toString() {
    ...
    if(this.name) {
      return `[object Field name="${this.name}" value=${value}]`;
    } else {
...
Field.prototype.string = Field.prototype.value;

for(let [name, func] of Object.entries(loaders)) {
  Field.prototype[name] = function(...optional) {
    return this.value(func, ...optional);
  }
}
