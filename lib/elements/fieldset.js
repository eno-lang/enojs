...
class EnoFieldset {
  ...
  element(name, options = { enforceElement: true }) {
    ...
    if(!this.entries_associative.hasOwnProperty(name)) {
      if(options.enforceElement) {
        throw errors.missingFieldsetEntry(this.context, name, this.instruction);
      }
  ...
  toString() {
    return `[object EnoFieldset name="${this.name}" entries=${this._entries.length}]`;
  }
...
EnoFieldset.prototype.string = EnoFieldset.prototype.entry;

for(let [name, func] of Object.entries(loaders)) {
  EnoFieldset.prototype[name] = function(name, ...optional) {
    return this.entry(name, func, ...optional);
  }
}

module.exports = EnoFieldset;
