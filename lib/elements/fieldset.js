...
class Fieldset {
  ...
  toString() {
    return `[object Fieldset name="${this.name}" entries=${this._entries.length}]`;
  }
...
Fieldset.prototype.string = Fieldset.prototype.entry;

for(let [name, func] of Object.entries(loaders)) {
  Fieldset.prototype[name] = function(name, ...optional) {
