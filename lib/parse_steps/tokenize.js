...
module.exports = context => {
  ...
  while(true) {
    ...
    if(match[matcher.EMPTY_LINE_INDEX] !== undefined) {
    ...
    } else if(match[matcher.DICTIONARY_ENTRY_EQUALS_INDEX]) {
      ...
      const startOfBlockIndex = index + 1;

      const nameEscaped = instruction.name.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&');
      const terminatorMatcher = new RegExp(`\\n[^\\S\\n]*(${blockDashes})[^\\S\\n]*(${nameEscaped})[^\\S\\n]*(?=\\n|$)`, 'y');
      ...
      while(true) {
        ...
        if(terminatorMatch) {
          ...
          if(line > instruction.line + 1) {
            instruction.contentRange = [startOfBlockIndex, terminatorMatch.index - 1];
          }
    ...
    } else if(match[matcher.TEMPLATE_INDEX]) {
      ...
      if(unescapedName) {
        ...
        const nameIndex = context.input.indexOf(instruction.name, index);
        copyOperatorIndex = context.input.indexOf('<', nameIndex + instruction.name.length);

        instruction.ranges = {
          copyOperator: [copyOperatorIndex - index, copyOperatorIndex - index + 1],
          name: [nameIndex - index, nameIndex - index + instruction.name.length]
        };
      } else {
        instruction.name = match[matcher.NAME_ESCAPED_INDEX];
        ...
        const escapeOperator = match[matcher.NAME_ESCAPED_QUOTES_INDEX];
        const escapeBeginOperatorIndex = context.input.indexOf(escapeOperator);
        const nameIndex = context.input.indexOf(instruction.name, escapeBeginOperatorIndex + escapeOperator.length);
        const escapeEndOperatorIndex = context.input.indexOf(escapeOperator, nameIndex + instruction.name.length);
        copyOperatorIndex = context.input.indexOf('<', escapeEndOperatorIndex + escapeOperator.length);
    ...
    } else if(match[matcher.COMMENT_ANGLE_INDEX]) {


      instruction.type = 'COMMENT';

      const operatorIndex = context.input.indexOf('>', index) - index;
      ...
      if(instruction.comment) {
        const textIndex = context.input.indexOf(instruction.comment, index + operatorIndex + 1) - index;
        instruction.ranges.comment = [textIndex, textIndex + instruction.comment.length];
      }
